...
*/
  ...
  function zoneinfo (zones, table) {
    ...
    while (table[0].charAt) abbrevs.push(table.shift());
    for (i = 0, I = Math.floor(table.length / 4); i < I; i++) {
      j = i * 4;
      zone.push(entry = {
        posix: Math.round(table[j] * 1e5),
        wallclock: Math.round((table[j] - table[j + 1]) * 1e5),
        save: Math.round(table[j + 2] * 6e5),
        abbrev: abbrevs[table[j + 3]],
        offset: offset
      });
      offset = entry.wallclock - entry.posix;
    }
    zone.push({ save: 0, abbrev: abbrevs[table.pop()], offset: offset });
