new function() {
  ...
  function recurse (request) {
    return convert.call(request, __slice.call(arguments, 1).concat([request.zone, request.locale]))
  }
  ...
  var FORMAT_RE = compileFormatRegEx();
  ...
  function format(request, wallclock, rest) {
    ...
    while (rest.length) {
      if (match = FORMAT_RE.exec(rest)) {
        if (match[4]) {
          var f = specifiers[match[4]];
          value = f(date, locale, request, match[3]);
          if (f.pad) {
            style = f.style || "0";
