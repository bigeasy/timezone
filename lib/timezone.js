new function() {
  var __slice = [].slice;
  var CLOCK, DAY, HOUR, MINUTE, SECOND, UTC, actualize, adjust, append, convert, convertToPOSIX, convertToWallclock, daysInMonth, die, en_US, extend, format, isLeapYear, iso8601, makeDate, offsetInMilliseconds, parse, parseAdjustment, say, search, tz, zoneinfo;
  die = function() {
    var splat;
  ...
  format = (function() {
    ...
    recurse = function() {
      var request, splat;
      request = arguments[0], splat = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
      return convert.call(null, null, request, splat.concat([request.zone, request.locale]), 0);
    };
    ...
    return function(request, wallclock, rest) {
  ...
  parseAdjustment = function(pattern) {
    var adjustment, count, match, plural, position, sign, unit, _ref;
    if (match = /^\s*(?:([+-]?)\s*(\d+)\s+)?(year|month|day|hour|minute|second|milli(?:second)?|sunday|monday|tuesday|wednesday|thursday|friday|saturday)(s)?(?:\s+(\d+))?\s*$/i.exec(pattern)) {
      _ref = match.slice(1), sign = _ref[0], count = _ref[1], unit = _ref[2], plural = _ref[3], position = _ref[4];
      if (position) {
        if (!count && !plural) {
          adjustment = {
            unit: unit,
  ...
  adjust = (function() {
    ...
    return function(request, posix, adjustment) {
      var count, date, fields, forward, increment, index, millis, month, offset, sign, unit, wallclock;
      sign = adjustment.sign, count = adjustment.count, unit = adjustment.unit;
      sign || (sign = "+");
      ...
      if (millis = TIME[unit]) {
        ...
        if (~(index = DAYS.indexOf(unit))) {
          while (offset !== 0) {
            ...
            if (new Date(wallclock).getUTCDay() === index) {
              offset--;
            }
          }
        } else if (unit === "day") {
        ...
        if (!((posix = convertToPOSIX(request, wallclock)) != null)) {
  ...
  append = function(context, request, value, key) {
    request[key] || (request[key] = extend({}, context[key]));
    return extend(request[key], value);
  };
  zoneinfo = function(context, table) {
    var abbrevs, entry, i, j, name, offset, zone, _i, _ref;
    zone = [];
    name = table.shift();
    offset = table.shift();
    abbrevs = [];
    while (typeof table[0] !== "number") {
      abbrevs.push(table.shift());
    }
    for (i = _i = 0, _ref = Math.floor(table.length / 4); 0 <= _ref ? _i < _ref : _i > _ref; i = 0 <= _ref ? ++_i : --_i) {
      j = i * 4;
      zone.push(entry = {
    ...
    return context.zones[name] = zone.reverse();
  };
  convert = function(tz, context, splat, length) {
    var adjustment, argument, count, date, index, key, object, partial, posix, request, token, type, wallclock, _i, _j, _k, _len, _len1, _len2, _ref, _ref1;
    request = {
      adjustments: []
    };
    count = 0;
    ...
    while (splat.length) {
      ...
      if (type === "number" && index === length) {
        ...
        if (index === length && (argument === tz.now || argument.getTime)) {
          request.date = argument;
    ...
    _ref = ["zones", "rules", "locales", "clock"];
    for (_j = 0, _len1 = _ref.length; _j < _len1; _j++) {
      key = _ref[_j];
      request[key] || (request[key] = context[key]);
    }
    if ((date = request.date) != null) {
      request.locale || (request.locale = "en_US");
      request.zone || (request.zone = "UTC");
      request.entry = UTC[0];
      if (!request.locales[request.locale]) {
      ...
      if (typeof date === "string") {
        ...
        posix = (request.clock || context.clock)();
      } else if (date.getTime) {
        posix = request.date.getTime();
      }
      _ref1 = request.adjustments;
      ...
      token = function() {
        var splat;
        splat = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
        return convert(token, request, partial.concat(splat), partial.length);
      };
      extend(token, tz);
    }
    return token;
  ...
  exports.tz = tz = (function() {
    ...
    return function() {
      var splat;
      splat = 1 <= arguments.length ? __slice.call(arguments, 0) : [];
      return convert(tz, context, splat, 0);
    };
  })();
  return tz.now = {};
}
