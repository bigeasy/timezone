(function () {
  ...
  function applicable (entry, rules, actualized, time, clock) {
    var i, I;
    var i, I, year = getYear(time);
    for (i = 0, I = rules.length; i < I; i++) {
      if (clock == "posix") year = getYear(time + entry.offset + rules[i].saved);
      if (rules[i].from <= year && year <= rules[i].to) {
        actualized.push(actualize(entry, rules[i], year, true));
      } else if (rules[i].to < year) {
        return rules[i].to;
      }
    }
  }
    ...
    if (typeof entry.rules == "string") {
      ...
      if (actualized.length) {
        ...
        for (var i = 0, I = actualized.length; i < I; i++) {
          ...
          if (time >= actualized[i][clock] && actualized[i][clock] > entry[clock]) found = actualized[i];
        }
      }
    }
    return found || entry;
  ...
  function verify (data, skipList, sorted, name)  {
    ...
    for (var i = 1, I = sorted.length; i < I; i++) {
      ...
      if (sorted[i].posix != found.posix) {
        ...
        say("YUP", iso8601(sorted[i].posix), iso8601(found.posix));
      }
    }
    return sorted;
  }
