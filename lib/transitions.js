(function () {
  ...
  function walk (data, begin, end, table) {
    ...
    if (begin[actualized[i].clock] == actualized[i][actualized[i].clock]) {
      actualized[i].rules = table.pop().rules;
      actualized[i].type = "entry";
      say("ACTUAMAMIZED", actualized[i]);
  ...
  function skipList (data, name, table) {
    ...
    for (i = 0, I = table.length; i < I; i++) {
      ...
      if (table[i].type == "rule") {
        ...
        if (getYear(table[i].posix) != getYear(table[i].wallclock)) {
  ...
  function applicable (entry, rules, actualized, time, clock) {
    ...
    for (var j = year + 1; j >= year - 1; --j) {
      for (i = 0, I = rules.length; i < I; i++) {
          say("year", clock, year, rules[i].from, rules[i].to, j, iso8601(time), time, entry.offset, rules[i].saved, iso8601(time + entry.offset + (rules[i].saved || 0)));
  ...
  function find (data, skipList, time, clock) {
    ...
    for (i = 0, I = skipList.length; i < I; i++) {
      say(iso8601(skipList[i][clock]), "<=",  iso8601(time));
      say(skipList[i].abbrev, skipList[i][clock], "<=",  time, skipList[i].abbrev, skipList[i][clock] <=  time);
    ...
    if (typeof entry.rules == "string") {
      ...
      if (actualized.length) {
        ...
        for (var i = 0, I = actualized.length; i < I; i++) {
          say(iso8601(time), actualized[i][clock], iso8601(actualized[i][clock]), time >= actualized[i][clock], clock, actualized[i].rule.saved, actualized.length);
          if (isNaN(actualized[i][clock])) say(actualized[i]);
          if (time >= actualized[i][clock] && actualized[i].wallclock > entry.wallclock) found = actualized[i];
        }
      }
    }
    return found || entry;
  ...
  function verify (data, skipList, sorted, name)  {
    ...
    for (var i = 1, I = sorted.length; i < I; i++) {
      ...
      if (sorted[i - 1].posix != found.posix) {
        ...
        say("YUP", iso8601(sorted[i - 1].posix), iso8601(found.posix));
      }
    }
    return sorted;
  }
  ...
  module.exports = function (data, zoneName, recurse) {
    ...
    say("ZONE IS: ", data.zones[zoneName]);
    table.push(entry);
    ...
    if (recurse) say(data.rules.Louisville);
