...
var DAY = "sun|mon|tue|wed|thu|fri|sat".split("|")
  ...
  // got them wrong, reversed from the actual time zones used by NATO. Thus, RFC
  // 2822 says that they can't be trusted. The only abbreviations that can be
  // trusted are the synonyms for UTC, and the abbreviations for the four time
  // zones of the contiguous United States. These were laid out in RFC 822.
  //
  // See [RFC 1123 5.2.14 Date and Time
  // Specification](http://www.ietf.org/rfc/rfc1123.txt).
  //
  // Otherwise, you cannot know the time zone offset from a date string found in
  // the wild, only in an RFC 822 date and only if is one of the following.
  , ABBREV =
    { Z: "+0000"
    , UT: "+0000"
    , GMT: "+0000"
...
// So, please, no pull requests with more time zone abbreviation mappings. What
...
// information confirming whether they are NATO or RFC 822. The second parameter
// to our parsing function is this optional out-of-band data.
//
...
function rfc822(date, abbrev) {
  ...
  // The full RFC 2822 specification includes support for features of legacy RFC
  // 822 dates that I've never see in the wild. Specifically the ability to
  // place comments anywhere in a date.
  //
  // > `Sun, 1 Mar (Spring is coming soon!) 92 00:00:00 GMT`
  //
  // Comments are enclosed in parenthesis. Comments can contain nested comments
  // making it impossible to match them with a regular expression, as any
  // discussion of a regular expression to match an RFC 822 email address will
  // tell you.
  //
  ...
  // If we cannot be certain of a time zone abbreviation, we default to "-0000"
  // which indicates an uncertain UTC, according to RFC 2822. Of course, that
  // bit of knowledge never leaves the function, but we'll just do as RFC 2822
  // says.
