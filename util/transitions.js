(function () {
  ...
  function actualize (entry, rule, year, saved) {
    ...
    if (saved) {
      ...
      // Oh, now. This is what it supposed to look like, this indent style. This
      // is even worse.
      actualized.clocks =
  ...
  function walk (_rules, begin, end, table) {
    ...
    // For each entry we generate every possible actualization. I do not
    // remember the reason for this overkill. Probably decided that ranging in
    // on the correct window was more logic than creating this table and not
    // worth it for a function that runs a couple times every few months.
    // Programmer efficency to slurp.
    ...
    // Okay. This should have done it. Resolved what happens when an zone entry
    // coincides with a rule entry.
    ...
    for (var i = 0, length = zone.length; i < length; i++) {
      ...
      if (previous.rules) {
        if (typeof previous.rules == "number") {
          var abbrevs
          if (abbrevs = /^([^\/]+)\/(.+)$/.exec(previous.format)) abbrevs.shift();
          else abbrevs = []
          previous.abbrev = abbrevs[1] || previous.format;
          previous.save = previous.rules * 6e4;
        } else {
          previous = walk(data.rules, previous, entry, table);
        }
      } else {
        previous.save = 0;
      }
